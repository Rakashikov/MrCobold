...
public class Player_Movement : MonoBehaviour
{
    ...
    #region Rotation and look Variable
    //Rotation and look
    float xRotation;
    ...
    Vector3 crouchScale = new Vector3(1, 0.6f, 1);
    Vector3 playerScale;
    #endregion

    #region Jumping Variable
    //Jumping
    [Header("Jumping")]
    ...
    #region Sliding Variable
    //Sliding
    Vector3 normalVector = Vector3.up;
    #endregion

    #region Particles Variable
    //Particles
    [Header("Particle")]
    ...
    void Awake()
    {
        ...
        if (landParticleObject != null) landParticle = landParticleObject.GetComponent<ParticleSystem>();
    }
    ...
    {
        Movement();
        if (grounded && rigidBody.velocity.magnitude > 5f) { isPlayAudio = true; }
        else { isPlayAudio = false; }
    }
    ...
    private IEnumerator PlaySound()
    {
        while (true)
        {
            ...
            if (isPlayAudio)
            {
                FindObjectOfType<AudioLevelManager>().ChangePitch("Step", Random.Range(0.8f, 1.2f));
                FindObjectOfType<AudioLevelManager>().StopPlay("Step");
                FindObjectOfType<AudioLevelManager>().Play("Step");
                isPlayAudio = false;
            }
            else
            {
    ...
    void OnCollisionEnter(Collision other)
    {
        ...
        for (int i = 0; i < other.contactCount; i++)
        {
            ...
            if (IsFloorAngle(normal))
            {
                ...
                grounded = true;
                if (!prevGrounded && grounded && yRbVel < -yVelToEffect) {
                    StopCoroutine("LandShake"); 
                    StartCoroutine(LandShake(yRbVel));
                }
                normalVector = normal;
            }
        }

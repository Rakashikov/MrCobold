...
public class Player_Wallrun : MonoBehaviour
{
    ...
    void StartWallRun()
    {
        ...
        if (rigidBody.velocity.magnitude > 8f) { isPlayAudio = true; }
    ...
    private IEnumerator PlaySound()
    {
        while (true)
        {
            ...
            if (isPlayAudio)
            {
                FindObjectOfType<AudioLevelManager>().ChangePitch("WallStep", Random.Range(0.8f, 1.2f));
                FindObjectOfType<AudioLevelManager>().StopPlay("WallStep");
                FindObjectOfType<AudioLevelManager>().Play("WallStep");
                isPlayAudio = false;
            }
            else
            {
